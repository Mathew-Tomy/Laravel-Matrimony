...
class HomeController extends Controller
{
    ...
    return view('frontend.member.member_listing.member', compact('users', 'age_from', 'age_to', 'member_code', 'matital_status', 'religion_id', 'caste_id', 'sub_caste_id', 'mother_tongue', 'profession', 'country_id', 'state_id', 'city_id', 'min_height', 'max_height', 'member_type','premium_members', 'new_members'));
    ...
    public function view_member_profile(Request $request,$id)
    {
        $similar_profiles = ProfileMatch::orderBy('match_percentage', 'desc')
            ->where('user_id', Auth::user()->id)
            ->where('match_id', '!=', $id)
        ...
        $existingView = ProfileView::where('visitor_id', Auth::id())
            ->where('user_id', $id)
            ->first();
...
// If a record doesn't already exist, insert a new one
        ...
        $uid = unique_notify_id();
        $notify_by = Auth::user()->id;
        $info_id = $uid;
        $message = Auth::user()->first_name . ' ' . Auth::user()->last_name . ' ' . translate('has visit your profile');
        $route = route('profile_visitors');

        // fcm 
        // if (get_setting('firebase_push_notification') == 1) {
        //     $fcmTokens = User::where('id', $id)->whereNotNull('fcm_token')->pluck('fcm_token')->toArray();
        ...
        Notification::send($notify_user, new DbStoreNotification($notify_type, $uid, $notify_by, $info_id, $message, $route));
        }catch (\Exception $e) {
            // dd($e);
        }
           }
